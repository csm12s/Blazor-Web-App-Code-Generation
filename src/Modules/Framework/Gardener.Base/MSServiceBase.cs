...
namespace Gardener
{
    ...
    public abstract class MSServiceBase<TEntity, TEntityDto, TKey, TDbContextLocator, TSlaveDbContextLocator> : IDynamicApiController, IServiceBase<TEntityDto, TKey>
        ...
        where TSlaveDbContextLocator : class, IDbContextLocator
    {
        ...
        /// TEntity Repository
        /// </summary>
        public readonly IMSRepository<TDbContextLocator, TSlaveDbContextLocator> _repository;
        /// <summary>
        /// 继承此类即可实现基础方法
        /// 方法包括：CURD、获取全部、分页获取 
        /// </summary>
        /// <param name="repository"></param>
        protected MSServiceBase(IMSRepository<TDbContextLocator, TSlaveDbContextLocator> repository)
        {
        ...
        public virtual async Task<TEntityDto> Insert(TEntityDto input)
        {
            ...
            var newEntity = await _repository.Master<TEntity>().InsertNowAsync(entity);
            //发送通知
        ...
        public virtual async Task<bool> Update(TEntityDto input)
        {
            ...
            EntityEntry<TEntity> entityEntry = await _repository.Master<TEntity>().UpdateExcludeAsync(input.Adapt<TEntity>(), new[] { nameof(GardenerEntityBase.CreatedTime), nameof(GardenerEntityBase.CreatorId), nameof(GardenerEntityBase.CreatorIdentityType) });
            //发送通知
        ...
        {
            await _repository.Master<TEntity>().DeleteAsync(id);
            //发送删除通知
        ...
        {
            await _repository.Master<TEntity>().FakeDeleteByKeyAsync(id);
        ...
        {
            var person = await _repository.Slave1<TEntity>().FindAsync(id);
        ...
        {
            var persons = _repository.Slave1<TEntity>().AsQueryable().Select(x => x.Adapt<TEntityDto>());
        ...
        public virtual async Task<List<TEntityDto>> GetAllUsable()
        {
            ...
            var persons = _repository.Slave1<TEntity>().AsQueryable().Where(where.ToString()).Select(x => x.Adapt<TEntityDto>());
            return await persons.ToListAsync();
        }
        ...
        {
            var pageResult = _repository.Slave1<TEntity>().AsQueryable();
        ...
        {
            var entity = await _repository.Master<TEntity>().FindAsync(id);
            if (entity != null && entity.SetPropertyValue(nameof(GardenerEntityBase.IsLocked), isLocked))
            {
                ...
                await _repository.Master<TEntity>().UpdateIncludeAsync(entity, new[] { nameof(GardenerEntityBase.IsLocked), nameof(GardenerEntityBase.UpdatedTime) });
        ...
        public virtual async Task<Base.PagedList<TEntityDto>> Search(PageRequest request)
        {
            ...
            IQueryable<TEntity> queryable = _repository.Slave1<TEntity>().AsQueryable(false).Where(expression);
