...
namespace Gardener.UserCenter.Client.Pages.LoginTokenView
{
    public partial class LoginToken
    {
        ITable _table;
        LoginTokenDto[] _datas;
        IEnumerable<LoginTokenDto> _selectedRows;
        int _total = 0;
        ...
        [Inject]
        public ILoginTokenService LoginTokenService { get; set; }
        /// <summary>
        /// 页面初始化完成
        /// </summary>
        /// <returns></returns>
        ...
        /// <returns></returns>
        private async Task OnChange(QueryModel<LoginTokenDto> queryModel)
        {
            await ReLoadTable();
        }
        /// <summary>
        ...
        private async Task ReLoadTable()
        {
            ...
            var pagedListResult = await LoginTokenService.Search(pageRequest);
        ...
        private async Task OnDeletesClick()
        {
            ...
            else
            {
                ...
                {
                    var result = await LoginTokenService.FakeDeletes(_selectedRows.Select(x => x.Id).ToArray());
                    if (result)
                    {
        ...
        private async Task OnDeleteClick(Guid id)
        {
            if (await confirmService.YesNoDelete() == ConfirmResult.Yes)
            {
                var result = await LoginTokenService.FakeDelete(id);
                if (result)
                {
        ...
        private async Task OnChangeIsLocked(LoginTokenDto model, bool isLocked)
        {
            var result = await LoginTokenService.Lock(model.Id, isLocked);
            if (!result)
            {
