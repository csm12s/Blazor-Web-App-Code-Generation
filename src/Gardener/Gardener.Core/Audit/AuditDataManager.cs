...
namespace Gardener.Core.Audit
{
    ...
    public class AuditDataManager : IAuditDataManager
    {
        ...
        public async Task SavingChangesEvent(DbContextEventData eventData, InterceptionResult<int> result)
        {
            ILogger<AuditDataManager> _logger = App.GetService<ILogger<AuditDataManager>>();
            try
            {
                // 获取当前事件对应上下文
       ...
       /// <param name="result"></param>
       /// <returns></returns>
        public async Task SavedChangesEvent(SaveChangesCompletedEventData eventData, int result)
        {
            ILogger<AuditDataManager> _logger = App.GetService<ILogger<AuditDataManager>>();
            try
            {
